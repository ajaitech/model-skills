...
- Runtime scope allows reads from `target_skill_directory` plus this skill's
  `references/` and `scripts/`, writes only to the target skill directory or
  `<target_skill_directory>/.tmp/skill-card-generator/`, and shell execution only
  for the three scripts listed below.
...
3. Stay within the declared scope. Do not read `.env`, credential files, hidden
   auth folders, or unrelated repo files; do not write outside the target skill
   directory or `<target_skill_directory>/.tmp/skill-card-generator/`.
...
run_script(
  ...
  args=[
    "--context", "<target_skill_directory>/.tmp/skill-card-generator/target-skill-context.json",
